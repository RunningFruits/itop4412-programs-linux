							...
							unsigned long arg)
{
	...
	case WDIOC_SETTIMEOUT:
	        if (copy_from_user(&new_margin, p,sizeof(p)))
	        return -EFAULT;
	        if (s3c2410wdt_set_heartbeat(new_margin))	
	        return -EINVAL;
		s3c2410wdt_keepalive();
		return copy_to_user(p,&tmr_margin,sizeof(tmr_margin));
...
//	dev_info(wdt_dev, "watchdog timer expired (irq)\n");
        gpio_direction_output(EXYNOS4_GPL2(0), (count++)%2);
	s3c2410wdt_keepalive();
	return IRQ_HANDLED;
}
...
static int __devinit s3c2410wdt_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(EXYNOS4_GPL2(0), "LED");
		if (ret) {
			printk("request GPIO %d for LED failed, ret = %d\n",EXYNOS4_GPL2(0), ret);
			return ret;
		}	 
